...
if (!file || ptrs.length === 0) {
  console.error("usage: check-paths <gs.txt|save.ck3> <ptr> [ptr...]");
  process.exit(2);
}

const buf = readFileSync(file);
const gsBuf = file.endsWith(".ck3")
  ? new AdmZip(buf.subarray(buf.indexOf(Buffer.from([0x50, 0x4b, 0x03, 0x04]))))
      .getEntries()
    ...
    const t =
      v instanceof Date
        ...
        : Array.isArray(v)
          ? `array[${v.length}]`
          : v && typeof v === "object"
            ? `object{${Object.keys(v).slice(0, 12).join(",")}}`
            : JSON.stringify(v);
    res[ptr] = `${typeof v}: ${t}`;
  }
  return { player, res };
