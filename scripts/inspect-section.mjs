...
if (!file || !needle) { console.error("usage: inspect-section <gs.txt|save.ck3> <needle> [bytes] [occurrence]"); process.exit(2); }
...
function loadText(f) {
  ...
  if (f.endsWith(".ck3")) {
    ...
    return new AdmZip(buf.subarray(at)).getEntries().find((e) => e.entryName === "gamestate").getData().toString("utf8");
  }
...
const re = new RegExp(`(^|\\n)\\t*${needle}=`, "g");
let m, found = 0, idx = -1;
while ((m = re.exec(gs)) !== null) { found++; if (found === occ) { idx = m.index + (m[1] ? 1 : 0); break; } }
if (idx === -1) { console.log(`'${needle}=' occurrence ${occ} NOT FOUND (total matches: ${found})`); process.exit(0); }
